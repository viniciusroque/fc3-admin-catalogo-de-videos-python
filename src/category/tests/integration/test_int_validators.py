...
class TestCategoryValidatorsIntegration(unittest.TestCase):
    ...
    def test_invalidation_cases_for_name_field(self):
        ...
        invalids_data = [
            ...
            {'data': {'name': None}, 'expected': 'This field may not be null.'}, #NOSONAR
        ...
        for invalid_data in invalids_data:
            is_valid = self.validator.validate(invalid_data['data'])
            self.assertFalse(is_valid)
            self.assertIn('name', self.validator.errors)
            self.assertListEqual(
                self.validator.errors['name'],
                [invalid_data['expected']],
                f"Expected: {invalid_data['expected']} actual: {self.validator.errors['name'][0]}"
            )
    ...
    def test_validate_cases(self):
        valid_data = [
            ...
            {'name': 'Movie', 'description': 'Some Value'},
            {'name': 'Movie', 'is_active': True},
            {'name': 'Movie', 'is_active': False},
            {'name': 'Movie', 'description': 'Some Value', 'is_active': True},
        ]

        for data in valid_data:
            is_valid = self.validator.validate(data)
            self.assertTrue(is_valid)
            self.assertEqual(self.validator.validated_data, data)
